...
export const GOLDEN_TESTS: GoldenTest[] = [
  {
    ...
    async execute(_runId, _client) {
      if (!VERCEL_GOLDEN_HEALTH_URL) return { passed: true, details: "VERCEL_GOLDEN_HEALTH_URL not set; skip" };
      try {
        ...
        return { passed: false, details: `Vercel health fetch failed: ${(e as Error).message}` };
      }
    },
  },
  {
    ...
    validatorType: "golden_test",
    async execute(_runId, _client) {
      if (!DNS_STAGING_VERIFY_HOST) return { passed: true, details: "DNS_STAGING_VERIFY_HOST not set; skip" };
      try {
        ...
        return { passed: false, details: `DNS verify failed: ${(e as Error).message}` };
      }
    },
  },
  {
    ...
    validatorType: "golden_test",
    async execute(_runId, _client) {
      if (!KLAVIYO_API_KEY) return { passed: true, details: "KLAVIYO_API_KEY not set; skip" };
      try {
        ...
        return { passed: false, details: `Klaviyo check failed: ${(e as Error).message}` };
      }
    },
  },
  {
    ...
    async execute(runId, client) {
      const r = await client.query<{ uri: string; artifact_type: string }>(
        "SELECT uri, artifact_type FROM artifacts WHERE run_id = $1",
        [runId]
      );
      ...
      if (mddLike.length === 0 && uris.length === 0) return { passed: true, details: "No artifacts to audit" };
  ...
  },
  {
    ...
    async execute(_runId, client) {
      const recipes = await client.query<{ error_signature: string }>("SELECT error_signature FROM repair_recipes LIMIT 1").catch(() => ({ rows: [] }));
      if (recipes.rows.length === 0) return { passed: true, details: "No repair_recipes; skip simulation" };
...
export async function runGoldenSuite(
  pool: pg.Pool,
  runId: string,
): Promise<{ allPassed: boolean; results: Array<{ name: string; passed: boolean; details: string }> }> {
  ...
  try {
    for (const test of GOLDEN_TESTS) {
      ...
      await client.query(
        `INSERT INTO validations (id, run_id, validator_type, status, created_at)
         VALUES ($1, $2, $3, $4, now())`,
        [uuid(), runId, test.validatorType, result.passed ? "pass" : "fail"],
      );

      if (result.artifactUri) {
        await client.query(
          `INSERT INTO artifacts (id, run_id, artifact_type, artifact_class, uri)
           VALUES ($1, $2, 'golden_validation', 'docs', $3)`,
          [uuid(), runId, result.artifactUri],
        );
      }
    }
  } catch (e) {
