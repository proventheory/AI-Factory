...
export async function routeRun(
  ...
  const routes = await pool.query<{
    ...
    `SELECT rule_id, release_id, cohort, percent
     FROM release_routes
     WHERE environment = $1
       AND (active_from IS NULL OR active_from <= now())
       AND (active_to IS NULL OR active_to > now())
     ORDER BY cohort, percent DESC`,
    [environment],
  );
  ...
  const latest = await pool.query<{ id: string }>(
    `SELECT id FROM releases WHERE status = 'promoted' ORDER BY created_at DESC LIMIT 1`,
  );
...
export async function computeDrift(
  ...
  const newSigs = await pool.query<{ error_signature: string }>(
    `SELECT DISTINCT jr.error_signature
     FROM job_runs jr
...
export async function executeRollback(
  ...
  await pool.query(
    `UPDATE releases SET status = 'rolled_back', percent_rollout = 0 WHERE id = $1`,
    [canaryReleaseId],
  );

  await pool.query(
    `UPDATE release_routes SET active_to = now()
     WHERE release_id = $1 AND environment = $2 AND active_to IS NULL`,
    [canaryReleaseId, environment],
  );

  // Generate incident artifact
  const incidentId = uuid();
  const latestRun = await pool.query<{ id: string }>(
    `SELECT id FROM runs WHERE release_id = $1 ORDER BY created_at DESC LIMIT 1`,
    [canaryReleaseId],
  );
  ...
  if (runId) {
    await pool.query(
      `INSERT INTO artifacts (id, run_id, artifact_type, artifact_class, uri, metadata_json)
       VALUES ($1, $2, 'drift_report', 'docs', $3, $4)`,
      [incidentId, runId, `mem://drift-report/${canaryReleaseId}`,
