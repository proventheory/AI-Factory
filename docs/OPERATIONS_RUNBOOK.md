...
**Staging self-heal when API is down:** The deploy-failure scan runs on both the **Control Plane** and the **Runner**. When **ai-factory-api-staging** has a failed deploy, the **runner** (if up) still runs the 5‑min scan and can trigger redeploys for api/gateway/runner — but only if the runner has **RENDER_STAGING_SERVICE_IDS** set to all three IDs (api, gateway, runner). If the runner only has **RENDER_WORKER_SERVICE_ID**, it monitors and heals only itself, so api-staging is never checked. Run `scripts/set-render-vercel-self-heal-env.mjs` so both API and runner get RENDER_STAGING_SERVICE_IDS; then redeploy the runner so it picks up the env. When **both** api-staging and runner-staging have failed deploys, nothing in that pair is running — the **backup Control Plane** (Render service ai-factory-api-prod) must run the scan. Set **RENDER_PROD_API_SERVICE_ID** (ai-factory-api-prod’s service ID from Render Dashboard) in `.env` and run `node --env-file=.env scripts/set-render-vercel-self-heal-env.mjs` once; the script pushes the same env to **prod** so prod’s 5‑min scan triggers staging redeploys. **Immediate trigger:** `POST https://<backup-control-plane-url>/v1/self_heal/deploy_failure_scan` (no body) once that service is deployed with the route. *(Everything is staging today; the backup Control Plane is just the healer instance.)* See [runbooks/render-staging-failed-deploy-and-duplicate-runner.md](runbooks/render-staging-failed-deploy-and-duplicate-runner.md) for "runner up but api-staging still not healing" and DEBUG_SELF_HEAL.
