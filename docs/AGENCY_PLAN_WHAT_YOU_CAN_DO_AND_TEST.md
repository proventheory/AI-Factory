...
ProfessorX is the **AI Factory Operator Console**: one control surface that talks to the Control Plane API and Postgres. It is **graph-native** (topology, lineage, upstream/downstream, repair scope, change impact) and includes **self-heal** (change events, graph impacts, incident memory, subgraph replay, repair preview, migration guard). You use it to:
...
- Inspect **graph health** (audit by run, missing capabilities by plan), **change impact**, **repair** (replay subgraph, similar incidents), and **migration guard** (SQL → risks, checkpoint suggestion); view **lineage** (producer + consumers) per artifact.
...
**In the Console you:** create initiatives, compile plans, view runs/jobs/artifacts/tool_calls, use the **Repair** tab on run detail (replay subgraph, Repair Preview), approve gates, set routing policies and LLM budgets, manage releases and canary, run self-heal, use **Graph** (Change Impact, Repair Preview, Migration Guard, Graph health), and use Admin for full CRUD.
...
**In the Console you:** manage brands and document templates, browse token and component registries, use the email marketing UI, track marketing deliverables as initiatives/plans/runs, and use graph/self-heal (change impact, repair, migration guard, lineage on artifacts) when relevant.
...
- **Deploy and Launches** – Use **Launches** (Orchestration → Launches): create a build spec from an initiative (form or strategy doc), then run a pipeline that produces a `launch_artifact`; trigger **Deploy preview** from the launch page (or use the `deploy_preview` job type so the runner proxies to the control-plane). Deploy targets: Vercel, Render, Netlify, Railway, Fly; DNS: Cloudflare or Vercel. You can also view the landing page via the artifact content URL without deploying.

---

## 3b. “So what can I actually do with what we have?”

**What works today (concrete)** — see table below. **Dependencies:** Runners must be running for real artifacts; migrations required for graph/self-heal.
...
| **Landing page** | **Initiatives (intent: landing)** → plan → run | Create a **landing** (or marketing) initiative, compile plan, start run; open **Artifacts** → **Open preview** for the landing page. |
| **Launches & deploy** | **Launches** (nav) or **Initiative → Launch** | Create build spec (form or strategy doc), get a launch; after a run produces a `launch_artifact`, trigger **Deploy preview** from the launch page. Optional: attach subdomain, validate, create repo. |
| **Graph & self-heal** | **Change Impact, Repair Preview, Migration Guard, Graph health** | Change impact by event; repair failed nodes (replay subgraph, similar incidents); migration SQL → risks; graph audit and missing capabilities. |
| **Lineage** | **Admin → Artifacts → open artifact** | See producer node and which job runs consumed the artifact. |

**Dependencies**

- **Runners must be running** for initiatives to turn into real artifacts (decks, copy, landing pages). Without that, the Console is **setup + tracking + graph ops**, not "click and get a deliverable."
- **Migrations** must be applied for graph/self-heal tables when using Change Impact, Repair, or Migration Guard.

**Bottom line**

With what we have: you **can** centralize client brands, define deck/report templates, use the email app, **spin up landing pages** via the landing pipeline, track campaigns as initiatives, and use **graph-native ops** (change impact, repair, migration guard, lineage, graph health). ProfessorX is the single Operator Console for pipelines, brands, and self-heal.
...
- **Graph & self-heal** – Change Impact, Repair Preview (replay subgraph, similar incidents), Migration Guard, Graph health/diagnostics; lineage per artifact (Admin → Artifacts).
...
2. **Console:** Open the Operator Console (ProfessorX) and confirm the new initiative (and plan/brand if created) appear.
