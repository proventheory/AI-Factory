...
**Name:** AI Factory / Operator Console full-system smoke test
**Goal:** Exercise the full stack (Control Plane API → Postgres) and confirm that data appears in the Operator Console (ProfessorX), so we know the system is wired end-to-end.

## What the script does

1. **Health check** — `GET /health` → confirms Control Plane is up.
2. **Create initiative** — `POST /v1/initiatives` with `intent_type: "software"`, title `"E2E smoke: AI Factory full-system test"`, `risk_level: "low"`. This writes to Postgres.
...
- **Health:** OK
- **Initiative:** Created (e.g. id `dd063649-5aa5-44d7-9e60-3c9baa145e6c`, title "E2E smoke: AI Factory full-system test")
...
1. Open the Operator Console (ProfessorX) (e.g. https://ai-factory-console-git-main-proventheorys-projects.vercel.app/ — log in if required).
2. Go to **Initiatives** (`/initiatives`). You should see **"E2E smoke: AI Factory full-system test"**.
...
## Klaviyo list endpoints (optional)

The smoke script also calls `GET /v1/klaviyo/templates`, `/v1/klaviyo/campaigns`, `/v1/klaviyo/flows`. These return 200 with `{ items: [] }` if the Klaviyo migrations are applied. If the tables do not exist yet, the requests may 500 (non-fatal). See [KLAVIYO_OPERATOR_PACK.md](KLAVIYO_OPERATOR_PACK.md) for full testing.
...
The task is: **create an initiative (and optionally a plan) via the API and confirm it shows up in the Operator Console.** The script automates the API side; you verify in the browser after logging in.
