 ...
 */
export async function runSweAgent(input: SweAgentInput): Promise<SweAgentOutput> {
  ...
  try {
    ...
    if (input.workspace_path) args.push("--repo_path", input.workspace_path);
    if (input.repo_url) args.push("--repo_path", input.repo_url);
...
function findPatchInDir(dir: string): string {
  try {
    const files = readdirSync(dir, { recursive: true }) as string[];
    const patchFile = files.find(f => f.endsWith(".patch") || f.endsWith(".diff"));
    if (patchFile) return readFileSync(join(dir, patchFile), "utf-8");
