 ...
 */
export async function runOpenHandsResolver(input: ResolverInput): Promise<ResolverOutput> {
  ...
  try {
    const args = [];
    if (input.issue_url) args.push("--issue-url", input.issue_url);
    if (input.workspace_path) args.push("--repo-path", input.workspace_path);
    if (input.repo_url) args.push("--repo-url", input.repo_url);
...
function extractPatchFromOutput(output: string): string {
  const diffStart = output.indexOf("diff --git");
  if (diffStart >= 0) return output.slice(diffStart);
  const unifiedStart = output.indexOf("--- a/");
  if (unifiedStart >= 0) return output.slice(unifiedStart);
  return "";
}
