/**
 * SEO target inventory: crawl target_url from goal_metadata, write seo_url_inventory artifact.
 */
import type pg from "pg";
import type { JobContext } from "../job-context.js";
import { crawlSite } from "../lib/seo/crawl.js";

export async function handleSeoTargetInventory(
  client: pg.PoolClient,
  context: JobContext,
  params: { runId: string; jobRunId: string; planNodeId: string },
): Promise<void> {
  const meta = context.goal_metadata ?? {};
  const targetUrl = (meta.target_url as string) ?? (meta.target as string);
  if (!targetUrl || typeof targetUrl !== "string") {
    throw new Error("seo_target_inventory requires goal_metadata.target_url");
  }
  const baseUrl = targetUrl.replace(/\/$/, "") || "https://example.com";
  ...
  const uri = `mem://seo_url_inventory/target/${context.run_id}/${context.plan_node_id}`;
  await client.query(
    `INSERT INTO artifacts (id, run_id, job_run_id, producer_plan_node_id, artifact_type, artifact_class, uri, metadata_json)
     VALUES (gen_random_uuid(), $1, $2, $3, 'seo_url_inventory', 'build_outputs', $4, $5::jsonb)`,
