            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="mt-2 max-w-3xl text-body-small text-fg-muted">
                  <strong>Run migration</strong> pushes selected sheets to Shopify via the pipeline runner: <strong>products</strong> (Woo → catalog, including tags), <strong>categories</strong> → custom collections, <strong>customers</strong>, <strong>redirects</strong> (Woo permalinks → Online Store redirects; set <strong>step 5</strong> target URL for destinations), <strong>discounts</strong> (simple percent / fixed_cart / fixed_product coupons → price rules + codes), <strong>pages</strong> (WP → Online Store pages), <strong>PDFs</strong> → Files, <strong>Blog posts</strong> → articles, <strong>Blog tags</strong> → redirect CSV. Shopify must be connected with Admin scopes for <strong>products</strong>, <strong>content</strong> (pages/blog), <strong>customers</strong>, <strong>discounts</strong>, and <strong>Online Store navigation</strong> (redirects) as needed. Each entity run is capped by the same batch limit as blog/PDF imports (from <strong>max_files</strong>, default 2000)—run again to continue. Variable products map Woo variations to Shopify options; grouped/external products are skipped with a row error. <strong>Product SEO:</strong> Yoast / Rank Math / AIOSEO title and meta description are read from Woo <code className="rounded bg-fg-muted/15 px-1">meta_data</code> and written to Shopify global SEO metafields; if the custom SEO title is empty in Woo, Shopify keeps the product name. Re-run with <strong>Skip if exists</strong> on to refresh SEO and tags on products already in Shopify. Use <strong>Details</strong> → uncheck rows to migrate only specific products. <strong>Blog posts</strong>: without a WP application password, only <strong>published</strong> posts import. PDFs upload sequentially; check Pipeline Runs if progress stalls.
                </p>
