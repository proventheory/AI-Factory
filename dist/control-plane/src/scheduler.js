...
export async function createRun(db, params) {
    ...
    for (const node of nodes.rows) {
        ...
        await db.query(`INSERT INTO node_progress (id, run_id, plan_node_id, deps_total, deps_satisfied, eligible_at, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`, [
        ...
        if (isRoot) {
           ...
           VALUES ($1, $2, $3, 1, 'queued', $4)`, [jobRunId, runId, node.id, idempotencyKey]);
            }
        }
    }
    if (hasQueuedJob) {
 ...
 */
export async function advanceSuccessors(db, runId, completedNodeId, winningJobRunId) {
    // Record completion (idempotent via ON CONFLICT DO NOTHING)
    await db.query(`INSERT INTO node_completions (run_id, from_node_id, job_run_id, completed_at)
     VALUES ($1, $2, $3, now())
     ON CONFLICT (run_id, from_node_id) DO NOTHING`, [runId, completedNodeId, winningJobRunId]);
    const successors = await db.query(`SELECT to_node_id FROM plan_edges
     WHERE plan_id = (SELECT plan_id FROM runs WHERE id = $1)
       AND from_node_id = $2
       AND condition = 'success'`, [runId, completedNodeId]);
    for (const { to_node_id } of successors.rows) {
        const result = await db.query(`UPDATE node_progress
       ...
       RETURNING deps_satisfied, deps_total`, [runId, to_node_id]);
 ...
 */
export async function completeApprovalAndAdvance(db, runId, planNodeId) {
    const jobRunId = uuid();
    await db.query(`INSERT INTO job_runs (id, run_id, plan_node_id, attempt, status, idempotency_key, started_at, ended_at)
     VALUES ($1, $2, $3, 1, 'succeeded', $4, now(), now())`, [jobRunId, runId, planNodeId, `approval:${runId}:${planNodeId}`]);
    await db.query(`INSERT INTO node_completions (run_id, from_node_id, job_run_id, completed_at)
     VALUES ($1, $2, $3, now())
 ...
 */
export async function checkRunCompletion(db, runId) {
    const result = await db.query(`SELECT
       count(*) as total,
       count(*) FILTER (WHERE status = 'succeeded') as succeeded
     FROM node_progress WHERE run_id = $1`, [runId]);
 ...
 */
export async function acquireRunLock(db, runId, lockDurationMs = 60_000) {
    const token = uuid();
    const expiresAt = new Date(Date.now() + lockDurationMs);
    const result = await db.query(`UPDATE runs
     SET scheduler_lock_token = $2, scheduler_lock_expires_at = $3
     WHERE id = $1
       AND (scheduler_lock_token IS NULL OR scheduler_lock_expires_at < now())
     RETURNING id`, [runId, token, expiresAt]);
    return result.rows.length > 0 ? token : null;
}
export async function releaseRunLock(db, runId, token) {
    await db.query(`UPDATE runs
     SET scheduler_lock_token = NULL, scheduler_lock_expires_at = NULL
     WHERE id = $1 AND scheduler_lock_token = $2`, [runId, token]);
}
