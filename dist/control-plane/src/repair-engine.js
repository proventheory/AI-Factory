...
export async function findRepairRecipes(pool, errorSignature, jobType, adapterId) {
    const result = await pool.query(`SELECT * FROM repair_recipes
...
export async function recordRepairOutcome(pool, recipeId, succeeded) {
    if (succeeded) {
        await pool.query(`UPDATE repair_recipes SET success_count = success_count + 1, last_used_at = now()
       WHERE id = $1`, [recipeId]);
    }
    else {
        await pool.query(`UPDATE repair_recipes SET failure_count = failure_count + 1, last_used_at = now()
       WHERE id = $1`, [recipeId]);
    }
}
/**
...
export async function promoteRepairRecipe(pool, errorSignature, patchPattern, validationRequired, createdFromJobRunId, jobType, adapterId) {
    const id = uuid();
    await pool.query(`INSERT INTO repair_recipes
       (id, error_signature, job_type, adapter_id, patch_pattern,
        validation_required, created_from_job_run_id, success_count)
     VALUES ($1, $2, $3, $4, $5, $6, $7, 1)`, [id, errorSignature, jobType ?? null, adapterId ?? null,
