...
const TEMPLATE_SEO_MIGRATION_AUDIT = {
    nodes: [
...
const TEMPLATE_SOFTWARE_DEPLOY = {
    nodes: [
...
const TEMPLATE_UPGRADE_INITIATIVE = {
    nodes: [
...
export function getTemplateByIntentType(intentType) {
...
export function computePlanHash(initiativeId, intentType, prdHashOrSeed) {
    const payload = `${initiativeId}:${intentType}:${prdHashOrSeed}`;
    return createHash("sha256").update(payload).digest("hex");
}
export function decomposeToDAG(initiative) {
    ...
    const templateId = initiative.intent_type === "email_design_generator" || initiative.intent_type === "email_campaign" /* backward compat */
...
export async function compilePlan(db, initiativeId, options) {
    ...
    let existing;
    try {
        existing = await db.query("SELECT id FROM plans WHERE initiative_id = $1 AND plan_hash = $2", [initiativeId, planHash]);
    }
    catch (err) {
    ...
    if (existing.rows.length > 0 && !options?.force) {
        ...
        try {
            nodes = await db.query("SELECT id, node_key FROM plan_nodes WHERE plan_id = $1", [planId]);
        }
        catch {
            await db.query("ROLLBACK TO SAVEPOINT before_existing_nodes");
            didRollbackNodes = true;
            const minimal = await db.query("SELECT id FROM plan_nodes WHERE plan_id = $1", [planId]);
            nodes = { rows: minimal.rows.map((r, i) => ({ ...r, node_key: `node_${i}` })) };
        }
        finally {
            if (!didRollbackNodes)
                await db.query("RELEASE SAVEPOINT before_existing_nodes");
        }
        const nodeIds = new Map();
        for (const n of nodes.rows)
            nodeIds.set(n.node_key, n.id);
    ...
    try {
        ...
        try {
            const versionResult = await db.query("SELECT coalesce(max(version), 0) + 1 AS v FROM plans WHERE initiative_id = $1", [initiativeId]);
            version = versionResult.rows[0]?.v ?? 1;
        }
        catch {
    ...
    try {
        ...
        try {
            await db.query("INSERT INTO plans (id, initiative_id, plan_hash, name, version) VALUES ($1, $2, $3, $4, $5)", [planId, initiativeId, planHash, initiative.title ?? null, version]);
        }
        catch (err) {
            if (err.code === "42703") {
    ...
    for (let i = 0; i < templateNodes.length; i++) {
        ...
        try {
            ...
            try {
                await db.query(`INSERT INTO plan_nodes (id, plan_id, node_key, job_type, node_type, agent_role, consumes_artifact_types, sequence)
           VALUES ($1, $2, $3, $4, $5::node_type, $6, $7, $8)`, [nodeId, planId, tn.node_key, tn.job_type, tn.node_type, tn.agent_role, tn.consumes_artifact_types ?? null, i + 1]);
            }
            catch (err) {
                if (err.code === "42703") {
                    await db.query(`ROLLBACK TO SAVEPOINT ${sp}`);
                    didRollbackNode = true;
                    await db.query("INSERT INTO plan_nodes (id, plan_id, node_key, job_type, node_type) VALUES ($1, $2, $3, $4, $5::node_type)", [nodeId, planId, tn.node_key, tn.job_type, tn.node_type]);
                }
                else
                    throw err;
            }
    ...
    for (const e of templateEdges) {
        ...
        try {
            ...
            try {
                await db.query("INSERT INTO plan_edges (id, plan_id, from_node_id, to_node_id, condition) VALUES ($1, $2, $3, $4, $5)", [uuid(), planId, fromId, toId, e.condition ?? "success"]);
            }
            catch (err) {
                if (err.code === "42703") {
                    await db.query(`ROLLBACK TO SAVEPOINT ${edgeSp}`);
                    didRollbackEdge = true;
                    await db.query("INSERT INTO plan_edges (id, plan_id, from_node_id, to_node_id) VALUES ($1, $2, $3, $4)", [uuid(), planId, fromId, toId]);
                }
                else
                    throw err;
            }
 ...
 */
export async function compilePlanFromDraft(db, initiativeId, draft, options) {
    ...
    if (!options?.force) {
        const existing = await db.query("SELECT id FROM plans WHERE initiative_id = $1 AND plan_hash = $2", [initiativeId, planHash]);
        if (existing.rows.length > 0) {
            const planId = existing.rows[0].id;
            const nodes = await db.query("SELECT id, node_key FROM plan_nodes WHERE plan_id = $1", [planId]);
            const nodeIds = new Map();
            for (const r of nodes.rows)
                nodeIds.set(r.node_key, r.id);
            return { planId, nodeIds, planHash };
        }
    }
    const { detectCycleFromTemplate } = await import("./graph/traversal.js");
    const { isKnownJobType, isValidEdgeCondition } = await import("./graph/schema.js");
    const templateNodes = draft.nodes.map((n) => ({
        node_key: n.node_key,
        job_type: n.job_type,
        agent_role: n.agent_role ?? "engineer",
        node_type: n.node_type ?? "job",
        consumes_artifact_types: n.consumes_artifact_types,
    }));
    const templateEdges = draft.edges.map((e) => ({
        from_key: e.from_key,
        to_key: e.to_key,
    ...
    if (!versionRolledBack)
        await db.query("RELEASE SAVEPOINT before_version").catch(() => { });
    let didRollbackPlans = false;
    try {
        await db.query("SAVEPOINT before_plans_insert");
        try {
    ...
    for (let i = 0; i < templateNodes.length; i++) {
        ...
        try {
           ...
           VALUES ($1, $2, $3, $4, $5::node_type, $6, $7, $8)`, [nodeId, planId, tn.node_key, tn.job_type, tn.node_type, tn.agent_role, tn.consumes_artifact_types ?? null, i + 1]);
            ...
            if (!didRollbackNode)
                await db.query(`RELEASE SAVEPOINT before_node_${i}`).catch(() => { });
        }
    }
    for (const e of templateEdges) {
        const fromId = nodeIds.get(e.from_key);
        ...
        try {
            ...
            if (!didRollbackEdge)
                await db.query(`RELEASE SAVEPOINT before_edge_${e.from_key}_${e.to_key}`).catch(() => { });
        }
    }
    return { planId, nodeIds, planHash };
}
//# sourceMappingURL=plan-compiler.js.map